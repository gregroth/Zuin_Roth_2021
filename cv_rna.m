%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%cv_rna - This function calculate the coefficient of variation of the
%number of mrna predicted by
...
function cv_rna =cv_rna(parm,cp)
nbs=ceil(parm(1));
kfar=parm(2);
kfor=parm(3);
kback=parm(4);
...
mudeltaH=parm(10);
    for i=1:length(cp)
        ...
        meanrna=trates*sd; %mean number of mrna
        m1=-((Q-eye(4*nbs))^(-1))'*diag(trates)*sd;
        vrna=meanrna-(meanrna)^2+trates*m1;%variance of the number of mrana, see eq. 7 in SI
        cv(i)=sqrt(vrna)/meanrna;
    end 
    cv_rna=cv';
end
